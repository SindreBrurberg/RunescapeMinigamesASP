...
namespace RunscapeMinigames
{
    ...
    class RunescapeMinigames
    {
		...
		private int getPoints(int[] tier, int[] tierUp, int totalUserXP, int gainedXP) {
			...
			for (int i = startTier; i <= tiers; i++) {
				if (startTier == tiers) {
					return (int)((double)gainedXP/tier[i]*100.0);
				} else {
					if (i==tiers) {
						points += (int)((double)XPLeft/tier[i]*100.0);
						return points;
					}
					Console.WriteLine(xpForLevel(tierUp[i]) - startXP);
					XPLeft -= xpForLevel(tierUp[i]) - startXP;
					points += (int)((double)(xpForLevel(tierUp[i]) - startXP)/tier[i]*100.0);
				}
			}
			return points;
		}
		private int points (string skillName, int totalUserXP, int gainedXP) {
			switch (skillName) {
				case "Ovreall":
					break;
				case "Attack":
